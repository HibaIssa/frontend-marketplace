  ...
  return (
    <div>
      ...
      <div className="p-6 flex flex-col gap-6">
        ...
        <CatalogPriceCharts volume={volume as { date: string; count: number }[]} />
              ...
              <tbody>
                {tableRows.map((change: any) => (
                  <tr key={`${change.product_id}-${change.recorded_at ?? change.last_seen ?? change.product_title}`} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-2 text-xs text-indigo-600 font-medium tabular-nums">
                      {formatCents(change.new_price)}
                    </td>
                    <td className="px-4 py-2">
                      <span className="text-xs font-semibold text-indigo-800 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-full">
