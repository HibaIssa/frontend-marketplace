...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      let res = await api.post<{ message?: string }>(endpoints.auth.resetPassword, {
        token: tokenFromUrl,
        password,
      })
      // Some backend deployments expose this as `/api/auth/reset`.
      if (!res.success && (res.error?.message?.toLowerCase().includes('not found') || res.error?.message?.includes('404'))) {
        res = await api.post<{ message?: string }>('/api/auth/reset', {
          token: tokenFromUrl,
          password,
        })
      }
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-3 top-[35px] text-neutral-500 hover:text-neutral-700"
