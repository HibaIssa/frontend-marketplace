          ...
          sections.map(({ section, items }) => (
            <div key={section} className="mb-4">
              ...
              {items.map(({ segment, label, icon: Icon, exact }) => {
                const localPath = segment === '' ? base : `${base}/${segment}`
                const href = hostedCatalogNavHref(localPath)
                const active = exact
                  ? pathname === localPath || pathname === `${localPath}/`
                  : pathname === localPath || pathname.startsWith(`${localPath}/`)
                return (
                  <Link
                    key={localPath}
                    href={href}
