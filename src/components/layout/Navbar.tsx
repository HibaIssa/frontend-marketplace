...
const ACTIVE_PILL = 'bg-[#2a2623] text-white shadow-sm'
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-5 lg:px-8 pt-3">
        <div
          className={clsx(
            ...
            scrolled
              ? 'bg-[#f7f5f3]/95 backdrop-blur-md shadow-[0_12px_28px_-22px_rgba(40,33,28,0.18)] ring-1 ring-[#d8d3cf]'
              : 'bg-[#f7f5f3]/90 backdrop-blur-sm ring-1 ring-[#d8d3cf]/80'
          )}
        >
          <Link href="/" className="flex items-center gap-2 shrink-0 group">
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#2a2623] text-white text-[0.72rem] font-extrabold tracking-tight">
              TZ
            </span>
            <span className="font-display text-[15px] font-bold text-[#1f1b18] tracking-tight">TrendZone</span>
          </Link>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Link href="/compare" className={clsx('relative hidden sm:inline-flex p-2 rounded-full transition-colors', pathname.startsWith('/compare') ? 'bg-slate-100 text-slate-900 ring-1 ring-slate-200' : 'text-slate-800 hover:bg-slate-100')} aria-label="Compare">
              ...
              {mounted && compareCount > 0 && <span className="absolute -top-0.5 -right-0.5 inline-flex h-4 min-w-[16px] items-center justify-center rounded-full bg-[#2a2623] text-white text-[10px] font-bold px-1 ring-2 ring-white">{compareCount}</span>}
            </Link>
            ...
            {mounted && isAuthenticated() ? (
              ...
              <div className="hidden sm:flex items-center gap-2">
                ...
                <Link href="/signup" className="text-[13px] py-2 px-4 rounded-full bg-[#2a2623] text-white hover:bg-[#43110e] transition-colors font-semibold">Sign up</Link>
              </div>
            )}
