...
export function TextSearchProductCard({
  ...
  const saveScroll = () => saveListingScrollY(fromReturnPath, typeof window !== 'undefined' ? window.scrollY : 0)
  ...
  return (
    <article className="group rounded-[18px] bg-white border border-[#ebe8e4] shadow-[0_6px_28px_-16px_rgba(42,38,35,0.12)] overflow-hidden transition-shadow duration-300 hover:shadow-[0_14px_40px_-18px_rgba(42,38,35,0.18)]">
      <Link href={href} className="block" prefetch={false} onClick={saveScroll} aria-label={product.title}>
      ...
      <div className="p-4 pt-3.5">
        <Link
          ...
          prefetch={false}
          onClick={saveScroll}
          className="block font-sans font-semibold text-[15px] sm:text-[16px] text-ink leading-snug line-clamp-2 hover:underline underline-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/35"
        >
          {product.title}
        </Link>
        <Link
          ...
          prefetch={false}
          onClick={saveScroll}
          className="mt-1 block text-small font-sans capitalize text-muted/90 line-clamp-1 underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/35"
        >
          {subtitle}
        </Link>
