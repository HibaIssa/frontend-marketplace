...
export function KpiCard({ label, value, sub, tone = 'default' }: KpiCardProps) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white/95 backdrop-blur-sm p-4 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_4px_24px_-4px_rgba(79,70,229,0.08)] hover:border-indigo-200/70 transition-colors">
...
export function PageHeader({ title, sub, actions }: PageHeaderProps) {
  return (
    <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-indigo-200/35 px-6 py-3.5 flex items-center justify-between">
      <div>
...
export function Skeleton({ className }: { className?: string }) {
  return <div className={clsx('bg-indigo-100/50 rounded animate-pulse', className)} />
}
    ...
    <button
      ...
      className={clsx(
        ...
        active
          ...
          : 'bg-white text-neutral-600 border-neutral-200 hover:border-indigo-300 hover:text-indigo-950'
      )}
    >
      {children}
    </button>
  ...
  if (!src) {
    return (
      <div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center border border-indigo-100">
        <span className="text-indigo-300 text-[10px]">img</span>
      </div>
    )
  }
  return (
