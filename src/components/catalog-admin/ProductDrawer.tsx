...
import type { Product, PriceHistory } from '@/types/catalog-admin'
import { Badge, AvailBadge } from '@/components/catalog-admin/ui'
import {
  formatCents,
  formatRelativeTime,
  getProductFlags,
...
export function ProductDrawer({ product: p, onClose }: Props) {
  ...
  useEffect(() => {
    if (tab === 'history' && history.length === 0) {
      setLoadingH(true)
      fetch(`/api/catalog/products/${p.id}/price-history`)
        .then((r) => r.json())
        .then((d) => setHistory(d ?? []))
                  ...
                  {styleData.recommendations.map((rec) => (
                    <div key={rec.category} className="bg-gray-50 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge
                          severity={
                            rec.priority === 1 ? 'critical' : rec.priority === 2 ? 'warning' : 'info'
                          }
                        >
                          {rec.priorityLabel}
                        </Badge>
